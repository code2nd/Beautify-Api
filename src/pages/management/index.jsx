...
const Management = (props) => {
  ...
  const handleOk = async () => {
    ...
    const { name } = SStorage.get('uploadedFileInfo');

    if (description) {
      if (isCover.current) {
        // 如果是覆盖文件，则需要先将服务器中该文件删除，然后再将新文件存储，然后修改数据库中该条记录
        await handleShearFile(name, description);
        isCover.current = false;
      } else {
        handlePostDocRecord(name, description);
  ...
  return (
    <div className="container" onClick={handleShowCustomPopover}>
      <main className="page-main">
              ...
              ? docList.map((item) => {
                  return (
                    <DocModule
                      ...
                      onDeleteClick={handleDeleteClick.bind(
                        this,
                        item.id,
                        item.name
                      )}
                      onDownloadJsonClick={handleDownloadJsonClick.bind(
                        this,
                        item.name
                      )}
                      onDownloadWordClick={handleDownloadWordClick.bind(
                        this,
                        item.name
                      )}
        ...
        >
          <Form form={form}>
            <Form.Item key="description" label="描述" name="description">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleDeleteFile() {
      dispatch(managementCreators.toDeleteFile());
    },
    ...
    handlePostDocRecord(name, description) {
      dispatch(managementCreators.toPostDocRecord(name, description));
    },
    handleShearFile(name, description) {
      dispatch(managementCreators.toShearFile(name, description));
    },
